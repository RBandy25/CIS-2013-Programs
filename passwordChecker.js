...
var passwordCheckOld = function () 
{
    ...
    //list of bad passwords that user is unable to use
    stringOutput = "";
   for (i=0; i<stringPassword.length; i++) //In order for the loop to be connected to the array, these are created. 
   //for loop that utilizes the array
   {
      if (stringPassword === arrayBadPasswords[i])
      {
         stringOutput = "The password entered is not correct."; //This demonstrates that the password is not correct because it falls under one of the bad passwords. 
         $("password").value = "";
      }
      else
      {
         if (stringPassword.length < 8) //This ensures that the oassword is not less than 8 characters. 
         {
            stringOutput = "I am sorry, the password is too short.";
         }
      }
      
   
      if  (stringOutput!= "The password entered is not correct." && stringOutput!= "I am sorry, the password is too short.") //These ensure that it is not correct; these alerts send messages to showt that the password input is wrong. 
         {
            stringOutput = "Yay! This is a great password.";
            $("password").value = "";
         }
